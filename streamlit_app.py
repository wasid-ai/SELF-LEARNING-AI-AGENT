...
def load_services():
    ...
    client = OpenAI(
        ...
        default_headers={
            ...
            "X-Title": "Self Learning AI Agent"
        }
    )
...
def needs_web_search(query):
    ...
    current_keywords = [
        ...
        "this week", "this month", "2026", "price", "prices", 
        "weather", "score", "result", "results", "new", "recently", 
        "bigg boss", "election", "stock", "market"
    ]
...
st.sidebar.info(f"Current User ID:\n\n**{user_id}**")
...
st.write("A general-purpose AI chatbot enhanced with long-term memory and free web search.")
st.caption("Mem0 + Qdrant + OpenRouter + Free Web Search")
...
if user_message:
    ...
    try:
        ...
        st.warning(f"Memory search failed: {e}")
    ...
    with st.chat_message("assistant"):
        with st.spinner("🤖 Thinking..."):
            try:
                ...
                answer = f"❌ AI response generate karne me error aaya:\n\n{e}"
...
st.sidebar.write("Each User ID has a separate memory space.")
st.sidebar.subheader("🌐 Web Search")
st.sidebar.write("Current information is searched using a free web-search layer.")
st.sidebar.divider()
st.sidebar.caption("Built with Python + Streamlit + Mem0 + Qdrant + OpenRouter + DDGS")
